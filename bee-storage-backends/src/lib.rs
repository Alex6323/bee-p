...
mod tests {
    ...
    fn create_random_tx() -> (bee_bundle::Hash, bee_bundle::Transaction) {
        let builder = bee_bundle::TransactionBuilder::default()
            .with_value(bee_bundle::Value(10))
            .with_address(bee_bundle::Address::from_str("ME"))
            .with_tag(bee_bundle::Tag::from_str("HELLO"))
            .with_nonce(bee_bundle::Nonce::from_str("ABCDEF"));

        (rand_hash(), builder.build().unwrap())
    }

    fn create_random_attached_tx(
    ...
    fn test_map_hashes_to_approvers() {
        ...
        for i in 0..2000 {
            let (tx_hash, mut tx) =
                create_random_attached_tx(last_tx_hash.clone(), last_tx_hash.clone());
    ...
    fn test_map_missing_transaction_hashes_to_approvers() {
        ...
        for i in 0..2000 {
            ...
            let (tx_hash, mut tx) = match i % 3 {
                ...
                1 => {
                    create_random_attached_tx(missing_tx_hash_branch.clone(), last_tx_hash.clone())
                }
                2 => create_random_attached_tx(
