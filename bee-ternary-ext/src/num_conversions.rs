...
pub fn i64_to_tritbuf(value: i64) -> TritBuf<T1B1Buf> {
    ...
    let mut buf = TritBuf::zeros(size);
...
pub fn tritbuf_try_to_i64(trits: TritBuf<T1B1Buf>) -> Result<i64, TritsI64ConversionError> {
    if trits.len() == 0 {
...
mod tests {
    ...
    fn round_robin() {
        ...
        for n in &nums {
            assert_eq!(tritbuf_try_to_i64(i64_to_tritbuf(*n)).unwrap(), *n);
        }
    }
    ...
    fn convert_range_to_trits() {
        ...
        for num in -100_000..100_000 {
            let buff = i64_to_tritbuf(num);
            let converted_num = tritbuf_try_to_i64(buff).unwrap();
    ...
    fn error_on_num_too_big() {
        ...
        assert_eq!(tritbuf_try_to_i64(buff).is_ok(), false);
    }
}
