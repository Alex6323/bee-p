...
fn test_new_seed() {
    ...
    assert_eq!(
        Ed25519PrivateKey::generate_from_seed(&seed, BIP32Path::from("m").unwrap()).unwrap().as_bytes(),
        Ed25519PrivateKey::generate_from_seed(&seed, BIP32Path::from("m").unwrap()).unwrap().as_bytes()
    );
    assert_eq!(
        Ed25519PrivateKey::generate_from_seed(&seed, BIP32Path::from("m/0H/1H/2H/2H/1000000000H").unwrap()).unwrap().as_bytes(),
        Ed25519PrivateKey::generate_from_seed(&seed, BIP32Path::from("m/0H/1H/2H/2H/1000000000H").unwrap()).unwrap().as_bytes()
    );
}
...
fn to_bytes_from_bytes() {
    ...
    let private_key1 = Ed25519PrivateKey::generate_from_seed(&seed1, BIP32Path::from("m/0H/2147483647H/1H/2147483646H/2H").unwrap()).unwrap();
...
fn seed_sign_and_verify() {
    let seed = Ed25519Seed::rand();
    let private_key = Ed25519PrivateKey::generate_from_seed(&seed, BIP32Path::from("m/0H/2147483647H/1H/2147483646H/2H").unwrap()).unwrap();
