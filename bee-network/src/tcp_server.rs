    ...
    loop {
        ...
        match stream_result {
            ...
            Ok(stream) => {
                ...
                spawn_and_log_error(read_task(Arc::clone(&stream), received_messages_clone, shutdown_server.clone().await));
                spawn_and_log_error(write_task(Arc::clone(&stream), messages_to_send_clone, shutdown_server_clone));
    ...
    loop {
        ...
        match TcpStream::connect(peer_config.address).await {
            ...
            Ok(stream) => {
                ...
                let received_messages_clone = received_messages.clone();
                let messages_to_send_clone = messages_to_send.clone().await;
                let mut shutdown_server_clone = shutdown_server.clone().await;
                ...
                spawn_and_log_error(read_task(Arc::clone(&stream), received_messages_clone, shutdown_server_clone.clone().await));
                spawn_and_log_error(write_task(Arc::clone(&stream), messages_to_send_clone, shutdown_server_clone));
...
async fn write_task(stream: Arc<TcpStream>, mut message_receiver: mpmc::Receiver<MessageToSend>, mut shutdown_server: mpmc::Receiver<()>) -> Result<(), Error> {
    ...
    loop {
        ...
        let message = select! {
            ...
            void = shutdown_server.next().fuse() => break
...
async fn read_task(stream: Arc<TcpStream>, mut received_messages: mpmc::Sender<ReceivedMessage>, mut shutdown_server: mpmc::Receiver<()>) -> Result<(), Error> {
