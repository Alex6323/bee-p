...
macro_rules! impl_transaction_ops {
    ($object:ty) => {
        ...
        impl $object {
            ...
            async fn set_solid_batch(hashes: &HashSet<Hash>, storage: &Storage) -> Result<(), OpError> {
                let mut batch = rocksdb::WriteBatch::default();
                let hash_to_solid = storage.inner.cf_handle(TRANSACTION_CF_HASH_TO_SOLID).unwrap();
                for hash in hashes {
                    let hash_buf = hash.to_inner().encode::<T5B1Buf>();
                    batch.put_cf(
                        &hash_to_solid,
